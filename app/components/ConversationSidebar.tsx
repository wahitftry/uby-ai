...
import { getDaftarPercakapan, hapusPercakapan, toggleBookmarkPercakapan, getDaftarPercakapanBookmark } from '../api/chatService';
...
export default function ConversationSidebar({
  ...
  return (
    <div className={`fixed inset-y-0 left-0 w-72 bg-background border-r border-black/10 dark:border-white/10 shadow-lg z-20 transform transition-transform duration-300 ${visible ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex flex-col h-full">        <div className="p-4 border-b border-black/5 dark:border-white/5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold">
              {tampilkanBookmark ? 'Percakapan Favorit' : 'Riwayat Percakapan'}
            </h2>
            <div className="flex items-center gap-1">
              <button 
                onClick={() => setTampilkanBookmark(!tampilkanBookmark)} 
                className={`rounded-full p-1.5 hover:bg-black/5 dark:hover:bg-white/5 ${
                  tampilkanBookmark ? 'text-yellow-500' : 'text-foreground/70'
                }`}
                title={tampilkanBookmark ? "Tampilkan semua percakapan" : "Tampilkan percakapan favorit"}
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill={tampilkanBookmark ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
