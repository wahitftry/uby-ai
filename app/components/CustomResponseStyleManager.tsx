...
const CustomResponseStyleManager: React.FC<CustomResponseStyleManagerProps> = ({
  ...
  const handleDelete = (id: string) => {
    ...
    confirmBox.innerHTML = `
      <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-fadeIn">
        ...
        <div class="p-5">
          <p class="mb-6">Apakah Anda yakin ingin menghapus gaya respons ini?</p>
  ...
  const filteredStyles = pencarian.trim() === '' ? daftarGayaResponsKustom : 
    daftarGayaResponsKustom.filter(gaya => 
      gaya.nama.toLowerCase().includes(pencarian.toLowerCase()) || 
      gaya.deskripsi.toLowerCase().includes(pencarian.toLowerCase()) ||
      gaya.petunjuk.toLowerCase().includes(pencarian.toLowerCase())
    );
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 flex items-center justify-center p-4 animate-fadeIn">
        <div className="bg-white/90 dark:bg-gray-900/90 rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden backdrop-blur-md">
          <div className="p-4 border-b border-black/10 dark:border-white/10 flex justify-between items-center bg-gradient-to-r from-white/50 to-white/30 dark:from-gray-800/50 dark:to-gray-900/30">
            <h2 className="text-lg font-semibold">Gaya Respons Kustom</h2>
            <button 
              onClick={onClose}
            ...
            <div className="space-y-3 max-h-[460px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-rounded scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700">
              {filteredStyles.length === 0 ? (
                <div className="text-center py-12 text-foreground/60">
                  ...
                  {pencarian ? 
                    'Tidak ada gaya respons yang cocok dengan pencarian.' : 
                    'Anda belum membuat gaya respons kustom.'}
                </div>
              ) : (
                filteredStyles.map((gaya) => (
                  <div key={gaya.id} className="border border-black/5 dark:border-white/10 rounded-xl p-4 hover:bg-white/80 dark:hover:bg-gray-800/80 transition-all bg-white/60 dark:bg-gray-800/40 group">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-medium">{gaya.nama}</h3>
                        {gaya.deskripsi && <p className="text-sm text-foreground/70 mt-0.5">{gaya.deskripsi}</p>}
                      </div>
