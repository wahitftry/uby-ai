  ...
  return (
    <div className="relative inline-block text-left">
      ...
      >
        ...
        </svg>      </button>      {menuTerbuka && (
        <div className="absolute right-0 bottom-full mb-1 w-60 origin-bottom-right rounded-xl shadow-lg border border-black/5 dark:border-white/5 bg-background backdrop-blur-lg z-20 max-h-[300px] overflow-y-auto">
          <div className="py-1">
            {daftarModel.map(model => (
              <button
                key={model.id}
                className={`block w-full text-left px-4 py-2 text-sm hover:bg-black/5 dark:hover:bg-white/5 transition-colors ${modelTerpilih === model.id ? 'text-blue-500 font-medium' : 'text-foreground/70'}`}
                onClick={() => {
      ...
      {menuTerbuka && (
        <div className="fixed inset-0 z-10" onClick={() => setMenuTerbuka(false)}></div>
      )}
    </div>
  );
};
